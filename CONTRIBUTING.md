...
The website is generated by `scripts/build_site.py` into `site/` (page content lives in that script; styles in `site/assets/site.css`). After editing:

```bash
python scripts/build_site.py
```

Commit `site/` and push to `main`. Vercel deploys it automatically to https://openwiki-delta.vercel.app/openwiki/. Links are absolute under `/openwiki/` (the `BASE` constant), so preview locally by serving a folder that contains `site/` as `openwiki/`. Photos are generated with `scripts/generate_images.py` (Gemini) and the share card with `scripts/build_og.py`.
